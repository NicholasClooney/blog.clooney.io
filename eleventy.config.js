...
const formatTimelineMonthLabel = (year, month) =>
...
const getTimelineWeekOfMonth = (date) => Math.floor((date.getUTCDate() - 1) / 7) + 1;

const formatTimelineWeekLabel = (year, month, week) =>
  `${formatTimelineMonthLabel(year, month)} · Week ${week}`;

const formatTimelineWeekRangeLabel = (year, month, week) => {
...
const formatCalendarWeekRangeLabel = (weekStart) => {
...
const buildTimelineMonthArchives = (entries) => {
  const archives = new Map();
  ...
  return Array.from(archives.values()).sort((a, b) => a.key.localeCompare(b.key));
};

const buildTimelineWeekArchives = (entries) => {
  const archives = new Map();
...
const buildTimelineCalendarWeekArchives = (entries) => {
  const archives = new Map();
